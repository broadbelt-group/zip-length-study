...
int main() {
    ...
    // Main KMC loop
    int chain_print_time = 15*60;           // want the first bond configuration to be saved at 15 minutes
    ...
    while (time_s < max_simulation_time) {
        ...
        if (time_s > csv_print_time) {
            ...
            save_state("checkpoint.h5", output_file, intact_chains, radical_chains, C_monomer, C_rad_RR, C_rad_BS, C_intact_chains, C_bonds, C_side_product, time_s, total_mass, repeat_unit_mass, end_group_mass, Mn, Mw);

            csv_print_time += 10;                   // print every 10 seconds
        }

        // If time is 0, 15, 30, 45, 60, 75 save weight of each chain
        if (time_s > chain_print_time){
            save_bond_distribution(intact_chains, time_s, repeat_unit_mass, end_group_mass);
            chain_print_time += 15*60; // increment by 15 mins
        }
    ...
    save_state("checkpoint.h5", output_file, intact_chains, radical_chains, C_monomer, C_rad_RR, C_rad_BS, C_intact_chains, C_bonds, C_side_product, time_s, total_mass, repeat_unit_mass, end_group_mass, Mn_final, Mw_final);
