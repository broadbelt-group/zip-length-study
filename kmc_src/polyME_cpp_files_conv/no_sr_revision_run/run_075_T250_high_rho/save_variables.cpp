...
void save_state(const std::string& filename, std::ofstream& output_file, std::vector<chain_struc>& intact_chains, std::vector<chain_struc>& radical_chains,
    ...
    double sp_wt_frac = (double) C_side_product * 396.18 / total_mass;
    
    // Write to output file
    output_file 
    << time_min << "," << monomer_yield << "," << polymer_wt_frac << "," << sp_wt_frac << "," << Mn << "," << Mw << "," 
    << C_monomer << "," << C_rad_RR << "," << C_rad_BS << "," << C_intact_chains  << "," << C_bonds << "," << C_side_product
    << std::endl;
    output_file.flush();
}
