...
int main() {
    ...
    // Start from scratch
    output_file << "Time (min),Monomer Conversion,Polymer Wt Fraction,Mn (g/mol),Mw (g/mol),"
    << "# Monomers,# Radical Chains RR,# Radical Chains BS,# Intact Chains,# Bonds"
    << std::endl;
    output_file.flush();
    ...
    // Write initial bond distribution (time = 0 min)
    save_bond_distribution(intact_chains, time_s, repeat_unit_mass, end_group_mass);
    ...
    while (time_s < max_simulation_time) {
        double total_propensity = calc_total_propensity(reaction_rates_and_params, C_rad_RR, C_rad_BS, C_bonds);
        ...
        if (!std::isfinite(tau)) {
            ...
            std::cerr << "C_rad_RR = " << C_rad_RR << ", C_rad_BS = " << C_rad_BS << ", C_bonds = " << C_bonds << std::endl;
            break;
        }
        ...
        if (time_s > csv_print_time) {
            ...
            // Calculate monomer conversion and weight fractions
            double monomer_conversion = static_cast<double>(C_monomer) / (total_mass / repeat_unit_mass);
            ...
            output_file << monomer_conversion << "," << polymer_wt_fraction << ","
                        << Mn << "," << Mw << ","
                        << C_monomer << "," << C_rad_RR << "," << C_rad_BS << ","
                        << C_intact_chains << "," << C_bonds << std::endl;
            output_file.flush();
        ...
        double current_conversion = static_cast<double>(C_monomer) / (total_mass / repeat_unit_mass);
    ...
    double monomer_conversion_final = static_cast<double>(C_monomer) / (total_mass / repeat_unit_mass);
    ...
    output_file << monomer_conversion_final << "," << polymer_wt_fraction_final << ","
                << Mn_final << "," << Mw_final << ","
                << C_monomer << "," << C_rad_RR << "," << C_rad_BS << ","
                << C_intact_chains << "," << C_bonds << std::endl;
    output_file.flush();
