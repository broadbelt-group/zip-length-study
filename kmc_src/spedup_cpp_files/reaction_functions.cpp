...
double calc_total_propensity(std::unordered_map<std::string, std::vector<double>>& reaction_rates, long& C_rad_RR, long& C_rad_BS, long& C_bonds) {
...
int weighted_random_selection(const std::vector<chain_struc>& intact_chains, int target_index, std::vector<long long>& cumulative_weights, long long sum_weights) {
    /* TEMP NEW: remove operation */
    // // Single pass: compute cumulative sum and select in one go
...
void initiation_random_scission(std::vector<chain_struc>& intact_chains, std::vector<chain_struc>& radical_chains, std::vector<long long>& cumulative_weights, std::vector<size_t>& removed_chain_idxs,
    ...
    // // DEBUG WEIGHTS
    // std::vector<long long> cumulative_weights_debug;
    //     cumulative_weights_debug.reserve(intact_chains.size());
    ...
    //             sum += chain[0];
    //         }
    //         cumulative_weights_debug.push_back(sum);
    //     }
    // assert(cumulative_weights_debug == cumulative_weights);

    // /* DEBUG: Assert statements! */
    // intact_chain_count = intact_chains.size();
    // for (auto& chain : intact_chains){
    //     if (chain[0] == -1){
    //         intact_chain_count--;
    //     }

    // }
    // assert(C_intact_chains == intact_chain_count);

    // assert(C_rad_RR == radical_chains.size());
    // mrad_count = 0;
    // non_mrad_count = 0;
    // for (size_t i = 0; i < radical_chains.size(); i++){
    //     if (radical_chains[i] == monomer_radical){
    //         mrad_count++;
    //     } else {
    //         non_mrad_count++;
    //     }
    // }
    // assert((mrad_count+non_mrad_count) == radical_chains.size());
    // assert(non_mrad_count == C_rad_BS);
    // bond_count = 0;
    // for (auto& chain : intact_chains){
    //     if (chain[0] != -1){
    //         bond_count += chain[0];
...
void save_bond_distribution(const std::vector<chain_struc>& intact_chains, double time_point, double repeat_unit_mass, double end_group_mass){
    // Convert seconds to mins
    int time_min = (int) time_point/60;

    // Create output filestream with specific filename
    std::string filename = "chain_weights_" + std::to_string(time_min) + "_mins.csv";
